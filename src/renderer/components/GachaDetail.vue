<template>
  <p class="text-gray-500 text-xs mb-2 text-center whitespace-nowrap">
    <span class="mx-2" :title="new Date(detail.date[0]).toLocaleString()">{{
      new Date(detail.date[0]).toLocaleDateString()
    }}</span>
    -
    <span class="mx-2" :title="new Date(detail.date[1]).toLocaleString()">{{
      new Date(detail.date[1]).toLocaleDateString()
    }}</span>
  </p>
  <p class="text-gray-600 text-xs mb-1">
    <span class="mr-1"
      >Total Wishes: <span class="text-blue-600">{{ detail.total }}</span>
    </span>
    <span v-if="type !== '100'"
      >Accumulated<span class="mx-1 text-green-600">{{ detail.countMio }}</span
      >Pity</span
    >
  </p>
  <p class="text-gray-600 text-xs mb-1">
    <span
      :title="`Character：${detail.count5c}\nWeapon：${detail.count5w}`"
      class="mr-3 whitespace-pre cursor-help text-yellow-500"
    >
      <span class="min-w-10 inline-block">5 Star：{{ detail.count5 }}</span>
      [{{ percent(detail.count5, detail.total) }}]
    </span>
    <br /><span
      :title="`Character：${detail.count4c}\nWeapon：${detail.count4w}`"
      class="mr-3 whitespace-pre cursor-help text-purple-600"
    >
  ...
  <p class="text-gray-600 text-xs mb-1" v-if="detail.ssrPos.length">
    ...
    <span
      :title="item[2]"
      class="cursor-help mr-1"
      :style="`color:${colorList[index]}`"
      v-for="(item, index) of detail.ssrPos"
      :key="item"
    >
      {{ item[0] }}[{{ item[1] }}]
    </span>
  </p>
  <p v-if="detail.ssrPos.length" class="text-gray-600 text-xs">
    Average For 5 Star：<span class="text-green-600">{{
      avg5(detail.ssrPos)
    }}</span>
  </p>
</template>
...
const colorList = computed(() => {
  ...
  props.data[1].ssrPos.forEach((item) => {
    ...
    const num = Math.abs(
      hashCode(`${Math.floor(Date.now() / (1000 * 60 * 10))}-${item[0]}`)
    );
