<template>
  <el-button
    ...
    >{{ state.status === "init" ? "Download Data" : "Update Data" }}</el-button
  >
  <el-button
    @click="saveExcel"
    class="focus:outline-none"
  ...
  <div v-if="detail" class="flex gap-4 flex-wrap justify-between cont">
    <div
      class="flex-grow flex-shrink-0 mb-4 w-64"
      v-for="(item, i) of detail"
      :key="i"
    >
    ...
    <i
      class="flex-grow flex-shrink-0 w-64 cat:hidden"
      v-for="i in 2"
      :key="i"
    ></i>
  </div>
...
const hint = computed(() => {
  if (state.status === "init") {
    return "Please open wish history and then click download data";
  } else if (state.status === "loaded") {
    return `Last Update：${new Date(state.data.time).toLocaleString()}`;
